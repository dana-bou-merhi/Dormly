...
import {
  Zap, Wifi, Dumbbell, ShieldCheck, ChefHat, Bus, BookOpen,
  ArrowRight, CheckCircle2, Sparkles, Users, TrendingDown,
} from "lucide-react";
...
const packages = [
  {
    ...
    btnClass: "border-teal-600 text-teal-600 hover:bg-teal-50",
    saveBadgeClass: "",
    // Maps to listing filters: price range + status
    filterParams: { maxPrice: "200", status: "Available Now" },
    perks: [
  ...
  },
  {
    ...
    filterParams: { minPrice: "150", maxPrice: "400", status: "Available Now" },
    perks: [
      { icon: Zap,         label: "24/7 Electricity backup", note: "Generator included on all floors",   color: "text-teal-600"   },
  ...
  },
  {
    ...
    filterParams: { minPrice: "400", type: "Luxury Apartment" },
    perks: [
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <li className="flex items-center gap-2.5 py-1 cursor-default group">
            ...
            <span className="flex-1 text-[13px] font-semibold text-gray-800 leading-snug">{label}</span>
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
          </li>
        </TooltipTrigger>
        <TooltipContent side="right" className="text-xs max-w-45">{note}</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
...
export default function StudentPackages() {
  ...
  const handleBrowse = (pkg) => {
    const params = new URLSearchParams({ package: pkg.id });
    if (pkg.filterParams.search)   params.set("search",   pkg.filterParams.search);
    if (pkg.filterParams.type)     params.set("type",     pkg.filterParams.type);
    if (pkg.filterParams.status)   params.set("status",   pkg.filterParams.status);
    if (pkg.filterParams.minPrice) params.set("minPrice", pkg.filterParams.minPrice);
    if (pkg.filterParams.maxPrice) params.set("maxPrice", pkg.filterParams.maxPrice);
              ...
              >
                ...
                <CardHeader className={`px-5 pt-3 pb-3 ${pkg.headerClass}`}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <h3 className="text-[17px] font-extrabold text-gray-900 leading-tight">{pkg.tier}</h3>
                    ...
                    <div className="text-right pt-1">
                      {pkg.monthlyAdd > 0 ? (
                        <>
                          ...
                          <span className={`text-2xl font-extrabold ${pkg.accentText}`}>+${pkg.monthlyAdd}</span>
                          <span className="text-xs text-gray-400">/mo</span>
                        </>
                      ) : (
                        <Badge variant="secondary" className="text-xs font-bold mt-1">Included</Badge>
                      )}
                    </div>
                  </div>
                  {pkg.savingsPct && (
                ...
                <CardContent className="px-5 py-1.5 flex-1">
                  <ul className="divide-y divide-gray-50">
                    {pkg.perks.map((perk) => <PerkRow key={perk.label} {...perk} />)}
                  </ul>
                </CardContent>
                ...
                <CardFooter className="px-5 py-3 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-xs text-gray-400">
                    ...
                    <span><strong className="text-gray-700">{pkg.listings}</strong> listings available</span>
                  </div>
                  {/* ── This is the key fix: onClick navigates with filters ── */}
                  <Button
                    size="sm"
                    onClick={(e) => { e.stopPropagation(); handleBrowse(pkg); }}
                    className={`text-xs font-bold rounded-lg ${pkg.btnClass}`}
                    variant={pkg.id === "essential" ? "outline" : "default"}
                  >
                    Browse {pkg.tier} →
                  </Button>
                </CardFooter>
